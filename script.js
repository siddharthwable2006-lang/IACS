...
<script>
    ...
    if (voltageSlider && voltageSetpoint) {
    ...
    function adjustVoltage(amount) {

        if (!voltageSlider || !voltageSetpoint) return;

        let value = parseFloat(voltageSlider.value);
    ...
    function applyVoltage() {
        ...
        const value =
            parseFloat(voltageSlider.value).toFixed(1);

        voltageSetpoint.textContent = value;
